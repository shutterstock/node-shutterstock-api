...
/*
 * Searches for an image or video and returns a read stream of results
 */
function searchStream(type, query, next) {
	query.page_number = 1;

	return search(type, query, function(err, data){
		var total = data.count || 0;
		var buffered = data.results;
		
		var stream = es.readable(function (i, callback) {
			// end the stream when we have read all results
			if(i > total ) { 
